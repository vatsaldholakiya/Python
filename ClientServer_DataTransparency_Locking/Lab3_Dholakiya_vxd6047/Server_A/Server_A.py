...
absPathToQueueDirectory_ServerA = "\\".join(
    os.getcwd().split('\\')[:-1]+["Server_A", "Queues"])
absPathToDirectoryA = "\\".join(os.getcwd().split(
    '\\')[:-1]+["Server_A", "parentDirectoryServer_A"])
absPathToDirectoryB = "\\".join(os.getcwd().split(
    '\\')[:-1]+["Server_B", "parentDirectoryServer_B"])
...
def getAbsoloutePathA(file): return os.path.join(absPathToDirectoryA, file)
# lambda function to get absoloute path for a particular file in server B
def getAbsoloutePathB(file): return os.path.join(absPathToDirectoryB, file)
# lambda function to check if the path exists
def doesPathExists(path): return os.path.exists(path)
# lambda function to get modified time
def modifiedTime(path): return os.path.getmtime(path)

def absoloutePathToQueue(file): return os.path.join(
    absPathToQueueDirectory_ServerA, file)
...
def syncServers():
    ...
    for file in os.listdir(absPathToQueueDirectory_ServerA):
        os.remove(absoloutePathToQueue(file))

    filesOnlyInA = [item for item in filesInServerA() if item not in filesInServerB()]
    print("fileList of A not B : \n\t\t", filesOnlyInA, '\n')
    # https://www.geeksforgeeks.org/python-intersection-two-lists/

    filesOnlyInB = [item for item in filesInServerB() if item not in filesInServerA()]
    print("fileList of B not A : \n\t\t", filesOnlyInB, '\n')
    # https://www.geeksforgeeks.org/python-intersection-two-lists/

    fileInAandB = [item for item in filesInServerA() if item in filesInServerB()]
    ...
    for file in filesOnlyInA:
        ...
        subprocess.run(['scp', filePathA, absPathToDirectoryB], shell=True, check=True)

    # send files only in B to A
    for file in filesOnlyInB:
        filePathB = getAbsoloutePathB(file)
        subprocess.run(['scp', filePathB, absPathToDirectoryA], shell=True, check=True)
    ...
    for file in fileInAandB:
        ...
        if fileModifiedTime_A > fileModifiedTime_B:
            print("\nFile : ", file, "\nmodifiedTime at A : ", time.ctime(
                fileModifiedTime_A), "\nmodifiedTime at B : ", time.ctime(fileModifiedTime_B))
            subprocess.run(['scp', getAbsoloutePathA(file),
                           absPathToDirectoryB], shell=True, check=True)
        elif fileModifiedTime_A < fileModifiedTime_B:
            print("\nFile : ", file, "\nmodifiedTime at A : ", time.ctime(
                fileModifiedTime_A), "\nmodifiedTime at B : ", time.ctime(fileModifiedTime_B))
            subprocess.run(['scp', getAbsoloutePathB(file),
                           absPathToDirectoryA], shell=True, check=True)
        else:
            None
...
def getDirectoryChange_A(initialListModifiedTime):
    ...
    for file in preExistingFiles:
        if modifiedTime(getAbsoloutePathA(file)) > initialListModifiedTime[file]:
...
def syncWithServerB(directoryChange_A, directoryChange_B):
    if len(directoryChange_A["addedFiles"]) + len(directoryChange_A["modifiedFiles"]) > 0:
        for fileName in directoryChange_A["addedFiles"] + directoryChange_A["modifiedFiles"]:            
            subprocess.run(['scp', getAbsoloutePathA(fileName), absPathToDirectoryB], shell=True, check=True)
    ...
    for file in modifiedFilesAtB:
        if oct(os.stat(getAbsoloutePathA(file)).st_mode & 0o777)[-3:] == '444':
            ...
            if file not in Queue.keys():
                ...
                Queue[file] = Queue[file] + directoryChange_B["modifiedFilesData"][file]
...
def getSortedFileListWithAttributes(fileList):
    ...
    for item in fileList:
        attributedFileList.append({
            ...
            "File": item,
            "Size": convert_bytes(os.path.getsize(getAbsoloutePathA(item))),
            "accessTime": time.ctime(os.path.getatime(getAbsoloutePathA(item))),
            "modifiedTime": time.ctime(modifiedTime(getAbsoloutePathA(item))),
            "assignedPermissions": oct(os.stat(getAbsoloutePathA(item)).st_mode)
        })
...
while (1):
    ...
    try:
        ...
        sortedExtendedFileListServerA = getSortedFileListWithAttributes(filesInServerA())

        if str(ex) == '\n\nRecieved request from client for locking/unlocking file.':
            args = json.loads(data)[-2:]
            file = getAbsoloutePathA(
                sortedExtendedFileListServerA[int(args[1])]['File'])
            if args[0] == '-lock':
            ...
            if args[0] == '-unlock':
                ...
                deployQueuedOperations(sortedExtendedFileListServerA[int(args[1])]['File'])
